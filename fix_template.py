        ...
        <div class="catalog-container">
            <aside class="catalog-sidebar">
                ...
                <div class="category-list">
                    <a href="{% url 'catalog:lista' %}"
                        class="category-item {% if not categoria_actual %}active{% endif %}">
                        Todos los productos
                    </a>
                    {% for categoria in categorias %}
                    <a href="{% url 'catalog:lista' %}?categoria={{ categoria.id }}"
                        class="category-item {% if categoria_actual == categoria.id|stringformat:'s' %}active{% endif %}">
                        {{ categoria.nombre }}
                    </a>
                    {% if categoria.subcategorias.all %}
                    <div class="subcategory-list" style="margin-left: 1rem;">
                    ...
                    {% endif %}
                    {% empty %}
                    <p style="color: var(--color-gray-500); font-size: 0.875rem;">No hay categorías</p>
                    {% endfor %}
                </div>
                        ...
                        <div class="filter-group">
                            ...
                            <div class="filter-options" id="filter-{{ filtro.nombre }}">
                                ...
                                <input type="text" class="filter-search" placeholder="Buscar..."
                                    data-target="filter-{{ filtro.nombre }}">
                                {% endif %}
                                {% for opcion in filtro.opciones %}
                                <label class="filter-option">
                                    <input type="checkbox" name="attr_{{ filtro.nombre }}" value="{{ opcion }}" {% if opcion in filtro.seleccionados %}checked{% endif %} onchange="document.getElementById('filtersForm').submit()">
                                    {{ opcion }}
                                </label>
                                {% endfor %}
                            </div>
                ...
                <div class="products-catalog-grid">
                    ...
                    <div class="product-catalog-card">
                        ...
                        <div class="product-info">
                            ...
                            <div
                                class="product-stock {% if producto.stock > 10 %}stock-available{% elif producto.stock > 0 %}stock-low{% else %}stock-out{% endif %}">
...
with open('templates/catalog/lista_fixed.html', 'w', encoding='utf-8') as f:
    f.write(content)

print("Archivo lista_fixed.html escrito correctamente.")
